...
locals {
  ...
  defaults = {
    label_order = [
      lower(module.tag_keys.GENERAL.NAMESPACE),
      lower(module.tag_keys.GENERAL.STAGE),
      lower(module.tag_keys.GENERAL.NAME),
      lower(module.tag_keys.GENERAL.ATTRIBUTES)
    ]
  ...
  string_label_names = [
    lower(module.tag_keys.GENERAL.NAME),
    lower(module.tag_keys.GENERAL.NAMESPACE),
    lower(module.tag_keys.GENERAL.ENVIRONMENT),
    lower(module.tag_keys.GENERAL.STAGE)
  ]
  ...
  non_null_tags = {
    for key in keys(local.input.tags) :
    key => local.input.tags[key]
    if local.input.tags[key] != null && local.input.tags[key] != ""
  }

  step1 = merge(local.generated_tags, local.non_null_tags)

  #add key tag by default if a name exists.
  step2 = merge(local.step1, var.name != null && var.name != "" ? {
    (module.tag_keys.GENERAL.KEY) : var.name
  } : {})

  tags = {
    for key in keys(local.step2) :
    key => local.step2[key]
    if local.step2[key] != null && local.step2[key] != ""
  }
  ...
  generated_tags = {
    ...
    local.label_key_case == "upper" ? (upper(l) == "NAME" ? "Name" : upper(l)) : (
      local.label_key_case == "lower" ? (lower(l) == "name" ? "Name" : lower(l)) : title(lower(l))
